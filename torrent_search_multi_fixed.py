...
Searches across all configured Jackett indexers with proper result filtering
"""
...
def search_all_indexers(query, category_filter=None):
    """Search across all configured indexers with result filtering"""
    ...
    for indexer in INDEXERS:
        ...
        if results:
            # Filter results by relevance before adding to all_results
...
def display_results(results, query):
    ...
    if not results:
        ...
        print(f"{Colors.PURPLE}Note: Indexers may be returning unrelated content{Colors.NC}")
        return
...
def main():
    """Main script logic"""
    print(f"{Colors.GREEN}=== Multi-Indexer Torrent Search Tool (Enhanced) ==={Colors.NC}")
    print(f"{Colors.BLUE}Available indexers: 1337x, eztv, thepiratebay, therarbg, yts{Colors.NC}")
    print(f"{Colors.PURPLE}Now with intelligent result filtering\!{Colors.NC}")
    print()
